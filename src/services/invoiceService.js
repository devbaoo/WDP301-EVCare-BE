...
const createFromAppointment = async (appointmentId, payload = {}) => {
    try {
        ...
        const quoteApproved = appointment?.inspectionAndQuote?.quoteStatus === 'approved';
        const quoteAmount = Number(appointment?.inspectionAndQuote?.quoteAmount || 0);
        const basePrice = Number(appointment?.serviceType?.pricing?.basePrice || 0);
        const estimatedCost = Number(appointment?.serviceDetails?.estimatedCost || 0);
        ...
        const doc = await Invoice.create({
            ...
            notes: payload.notes || `appointmentId=${appointment._id}; source=${quoteApproved && quoteAmount > 0 ? 'quote' : (basePrice > 0 ? 'serviceType' : 'estimatedCost')}`,
        });

        return { success: true, statusCode: 201, message: "Tạo hóa đơn nháp thành công", data: doc };
    } catch (error) {
...
const sendInvoiceEmail = async (invoiceId) => {
    try {
        const Invoice = await loadInvoiceModel();
        const invoice = await Invoice.findById(invoiceId).populate("customerId", "fullName username email");
        if (!invoice) return { success: false, statusCode: 404, message: "Không tìm thấy hóa đơn" };

        // Thử lấy appointmentId từ notes nếu có dạng 'appointmentId=...'
        let appointment = null;
        try {
            const match = String(invoice.notes || "").match(/appointmentId=([a-f\d]{24})/i);
            if (match && match[1]) {
        ...
        return { success: true, statusCode: 200, message: "Gửi email hóa đơn thành công" };
    } catch (error) {
        console.error("Send invoice email error:", error);
        return { success: false, statusCode: 500, message: "Lỗi khi gửi email hóa đơn" };
    }
};
