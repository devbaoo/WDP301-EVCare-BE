...
const getAllServiceCenters = async (filters = {}) => {
  try {
    ...
    // Hydrate staff from StaffAssignment to ensure consistency
    const centersWithStaff = await Promise.all(
      serviceCenters.map(async (center) => {
        ...
        const staff = assignments.map((a) => ({
          user: a.userId, // populated user document
          role: a.position,
          isActive: a.isActive,
        }));

        const centerObj = center.toObject();
        centerObj.staff = staff;
        return centerObj;
      })
    );
    ...
    return {
      ...
      data: {
        serviceCenters: centersWithStaff,
        pagination: {
          currentPage: page,
...
const getServiceCenterById = async (id) => {
  try {
    const serviceCenterDoc = await ServiceCenter.findById(id).populate(
      "services",
      "name category pricing.basePrice description"
    );
    ...
    // Hydrate staff from StaffAssignment
    const assignments = await StaffAssignment.find({
      centerId: serviceCenterDoc._id,
      isActive: true,
    })
      .populate("userId", "username fullName email avatar role")
      .sort({ position: 1, createdAt: -1 });
    ...
    const serviceCenter = serviceCenterDoc.toObject();
    serviceCenter.staff = staff;
...
const findNearestCenters = async (lat, lng, radius = 10) => {
  try {
    const serviceCentersDocs = await ServiceCenter.find({
      status: "active",
    ...
    const serviceCenters = await Promise.all(
      serviceCentersDocs.map(async (center) => {
        const assignments = await StaffAssignment.find({
          centerId: center._id,
          isActive: true,
        })
